...
**Run an AI character as a series, not a render.** Character sheet → lore → numbered episodes → Google
Flow (Nano Banana + Omni / Veo) driven through a real browser → ffmpeg assembly → account warm-up.
...
```
bible ──▶ sheet ──▶ character in Flow ──▶ episode.json ──▶ flow.mjs batch ──▶ assemble.sh ──▶ post ──▶ measure
```

| Step | Tool | Cost |
|---|---|---|
| character sheet, 2 variants | Nano Banana Pro in Flow | 0 credits on PRO |
| 3 shots x 2 takes, 8 s, 720p | Omni 1.1 Flash in Flow | 72 credits (~$1.44 at PRO's $19.99 / 1,000) |
